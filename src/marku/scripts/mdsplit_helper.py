...
def main():
    ...
    if not args.input or args.input == "-":
        args.input = r"D:\1VSCODE\Projects\MarkdownAll\MarkdownWrapper\src\marku\scripts\1.md"
    ...
    if Prompt is not None and sys.stdin.isatty():
        # 仅在交互式终端下才弹出 rich 交互
        if args.input == r"D:\1VSCODE\Projects\MarkdownAll\MarkdownWrapper\src\marku\scripts\1.md":
            console = Console()
            console.print("[bold green]未检测到输入文件路径，将进入交互式输入模式：[/bold green]")
            args.input = Prompt.ask("请输入 Markdown 文件路径或文件夹路径 (输入 '-' 代表标准输入)", default=args.input)
        if not args.output:
            args.output = Prompt.ask("请输入输出目录 (可留空自动生成)", default=None)
        if not args.encoding:
            args.encoding = Prompt.ask("请输入文件编码 (可留空使用默认)", default=args.encoding)
        if not args.max_level:
            args.max_level = int(Prompt.ask(f"请输入最大分割级别 (1-{MAX_HEADING_LEVEL})", default=str(args.max_level)))
        if not args.table_of_contents:
            args.table_of_contents = Prompt.ask("是否生成目录 toc.md? (y/n)", default="n").lower() == "y"
        if not args.navigation:
            args.navigation = Prompt.ask("是否添加导航页脚? (y/n)", default="n").lower() == "y"
        if not args.force:
            args.force = Prompt.ask("输出目录已存在时是否强制写入? (y/n)", default="n").lower() == "y"
        if not args.verbose:
            args.verbose = Prompt.ask("是否显示详细信息? (y/n)", default="n").lower() == "y"
    elif Prompt is None and (not args.input or args.input == "-"):
        print("未安装 rich，无法使用交互式输入。请通过命令行参数提供输入文件路径。\n可通过 'pip install rich' 安装 rich。")
