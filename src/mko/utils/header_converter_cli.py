...
def setup_logging():
    ...
    log_file = os.path.join(log_dir, f"header_transformer_{timestamp}.log")
...
class HeaderTransformerWrapper(BaseMarkoTransformer):
    """标题转换器包装类，增加统计功能"""
    
    def __init__(self, header_levels=None):
        """初始化转换器"""
        super().__init__()
        self.header_transformer = HeaderTransformer(header_levels)
        
    def render(self, document):
        """渲染文档"""
        return self.header_transformer.render(document)
...
def process_file(file_path, header_levels=None):
    ...
    try:
        ...
        transformer = HeaderTransformerWrapper(header_levels)
        transformed_content = transformer.transform(content)
        end_time = time.time()
        ...
        if transformed_content != content:
            ...
            stats["headers_converted"] += 1  # 这个统计不太准确，只是表示文件有变化
            
            # 显示处理结果
            new_length = len(transformed_content)
            ...
            print(f"  - 文件已更新{Style.RESET_ALL}")
        else:
            print(f"{Fore.YELLOW}完成: {os.path.basename(file_path)} - 未发现需要转换的标题{Style.RESET_ALL}")
...
def main():
    ...
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='基于marko的Markdown标题转换工具')
    ...
    try:
        ...
        if os.path.isfile(path):
            ...
            success_count = process_directory(path, header_levels, recursive)
            ...
            print(f"转换标题数量: {stats['headers_converted']}")
...
if __name__ == '__main__':
    print(f"{Fore.CYAN}===== Markdown 标题转换工具 ====={Style.RESET_ALL}")
    main()
